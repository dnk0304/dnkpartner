...
def create_flyer():
    ...
    filename = os.path.join(user_home, "Downloads", "viken_promo_flyer_devices.png")
    ...
    # REMOVED "Har du binding?" text here.
    ...
    # --- New Usage Example Section (500 Mbps capacity) ---
    y_cursor += 100
    
    # Draw a highlighted box for the speed example
    speed_box_h = 160
    draw.rounded_rectangle([(80, y_cursor), (width - 80, y_cursor + speed_box_h)], radius=20, fill=(0, 43, 73), outline="#00AEEF", width=2)
    
    # Icon placeholder (Bolt)
    draw.ellipse([(110, y_cursor + 30), (210, y_cursor + 130)], fill="#00AEEF")
    draw.text((140, y_cursor + 50), "500", fill=COLOR_WHITE, font=get_font(30, True))
    
    # Text explanation
    draw.text((240, y_cursor + 25), "Hva betyr 500/500 Mbps for deg?", fill=COLOR_WHITE, font=font_body_bold)
    
    example_text = "Nok til at hele familien kan surfe, game og streame\n4K på 20+ enheter samtidig – uten forsinkelser!"
    draw.text((240, y_cursor + 80), example_text, fill="#CCCCCC", font=font_small)

    y_cursor += 200

    # --- List Items ---
    # Removed "Symmetrisk Fiber" line since it's highlighted above now, or keep as summary?
    # User wanted to "include an example", didn't strictly say replace. 
    # But list might be redundant. Let's keep other items.
