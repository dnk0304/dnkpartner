...
def create_flyer():
    ...
    filename = os.path.join(user_home, "Downloads", "viken_promo_flyer_final.png")
    ...
    bx = width - w_b - 80
    by = 40 # Moved to very top right to avoid overlap
    ...
    y_cursor += 380 # Reduced from 420

    # --- Feature Grid (Reduced Spacing) ---
    y_cursor += 100 # Reduced from 150
    ...
    draw_badge_clean(80, y_cursor, "60+ TV-kanaler", "Velg fritt i portalen", COLOR_RED)
    draw_badge_clean(560, y_cursor, "10+ Strømmetjenester", "Netflix, HBO m.fl.", "#00AEEF")
    
    y_cursor += 160 # Reduced from 170
    ...
    # --- Devices Strip (Reduced spacing) ---
    y_cursor += 160 # Reduced from 180
    draw.text((80, y_cursor), "Se overalt med Altibox-appen:", fill="#FFFFFF", font=font_body_bold)
    y_cursor += 50 # Reduced from 60
    ...
    # --- KSPs & Usage Example (Split Layout, Reduced spacing) ---
    y_cursor += 100 # Reduced from 120
    
    # Left Side: KSPs (Bullet points) - Added "60 poeng"
    features = [
        "Symmetrisk Fiber 500/500",
        "TV-pakke: 60 valgfrie poeng",
    ...
    # Adjusted title and text sizing
    box_x = 560
    box_w = 440
    box_h = 270 # Increased height to prevent text overlap
    
    draw.rounded_rectangle([(box_x, y_cursor), (box_x + box_w, y_cursor + box_h)], radius=20, fill=(0, 0, 0, 100), outline="#00AEEF", width=2)
    
    # Title inside box - Changed as requested
    draw.text((box_x + 20, y_cursor + 15), "Er 500 Mbps nok?", fill="#00AEEF", font=font_body_bold)
    
    # List - Font size adjusted to prevent overlap
    usage_list = [
        ("10 x Smart-TV (4K)", "250 Mbps"), # Shortened text to prevent overlap
        ("5 x PC (Videomøter)", "50 Mbps"), # Shortened text
        ("10 x Mobil (Sosialt)", "50 Mbps"), # Shortened text
    ]
    
    uy = y_cursor + 60
    font_list_item = get_font(24) # Slightly smaller
    font_list_val = get_font(24, True)

    for label, val in usage_list:
        draw.text((box_x + 20, uy), label, fill="#DDDDDD", font=font_list_item)
        # Value right aligned in box
        left, top, right, bottom = draw.textbbox((0, 0), val, font=font_list_val)
        wv = right - left
        draw.text((box_x + box_w - wv - 20, uy), val, fill=COLOR_WHITE, font=font_list_val)
        uy += 35 # Increased slightly for readability
    ...
    draw.text((box_x + 20, uy + 45), "Fortsatt masse ledig kapasitet!", fill="#4CAF50", font=get_font(20, True))

    y_cursor = max(ksp_y, y_cursor + box_h) + 40

    # --- Streaming Logos ---
    draw.text((80, y_cursor), "Velg dine favoritter:", fill="#FFFFFF", font=font_body_bold)
    y_cursor += 60 # Reduced from 70
