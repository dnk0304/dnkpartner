...
const AuctionLocationMap = dynamic(
    ...
    ),
  {
    ssr: false,
    loading: () => (
      <div className="h-full w-full animate-pulse bg-[--color-surface-muted]" />
    ),
  },
);
...
export function AuctionMapPanel({
  ...
  const mapFrame =
    variant === "hero" ? (
      <div className="relative aspect-[16/9] w-full overflow-hidden rounded-lg border border-[--color-hairline]">
        <AuctionLocationMap auction={auction} zoom={16} />
      </div>
    ) : (
      <div className="overflow-hidden rounded-lg border border-[--color-hairline]">
    ...
    <div className={cn("space-y-3", className)}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0 flex-1">
          {auction.address && (
            <p className="flex items-start gap-1.5 text-sm text-[--color-ink-secondary]">
              <MapPin
                className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[--color-ink-tertiary]"
                aria-hidden="true"
              />
              <span className="break-words">{auction.address}</span>
            </p>
          )}
          <p className="mt-1 text-[11px] text-[--color-ink-tertiary]">
        ...
        <div className="flex flex-wrap items-center gap-2 shrink-0">
          {gmapsHref && (
            <a
              ...
              className={cn(
                "inline-flex items-center gap-1.5 rounded-md border border-[--color-hairline] bg-[--color-surface] px-3 py-1.5 text-xs font-medium text-[--color-ink-secondary]",
