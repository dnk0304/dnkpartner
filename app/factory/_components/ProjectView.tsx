...
/**
 * Trigger a client-side download of base64-encoded binary (no deps). Decodes the
 * base64 → bytes → typed Blob → object URL → click → revoke, mirroring
 * downloadText. Used for the Stage-3 live-formula .xlsx workbook, whose bytes the
 * artifact payload carries as base64 (no separate file fetch needed).
 */
 ...
 * its markdown. Absent entirely for non-computational products. Narrowed from the
 * opaque Json payload — we only surface the Download .xlsx button when a real,
 * computational workbook with bytes is present (never a dead/fake button).
 */
...
function readWorkbook(payload: unknown): ArtifactWorkbook | null {
  ...
  return {
    ...
    sheetNames: Array.isArray(sheetNames) ? sheetNames.filter((s): s is string => typeof s === 'string') : [],
    computational: true,
  };
}
...
function StageSection({ artifact }: { artifact: Artifact }) {
  ...
  const downloadWorkbook = () => {
    if (!workbook) return;
    // The producer's own filename, else a safe fallback off the artifact title.
    const fallback =
      ...
      >
        ...
        {!isEmpty && (
          <div className="flex shrink-0 flex-wrap items-center justify-end gap-2">
            {workbook && (
              <span className="inline-flex items-center rounded-full bg-brand-light px-2 py-0.5 text-[11px] font-medium text-brand-primary">
                {workbook.sheetNames.length} {workbook.sheetNames.length === 1 ? 'tab' : 'tabs'} · live formulas
              </span>
            )}
            <button
              type="button"
              onClick={download}
              className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-brand-dark/70 shadow-sm transition-colors hover:bg-slate-50 hover:text-brand-accent focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary/40"
            >
