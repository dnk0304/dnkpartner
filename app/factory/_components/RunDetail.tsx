    ...
    <div className="fixed inset-0 z-40 flex justify-end" role="dialog" aria-modal="true" aria-label={`${detail.seed} project detail`}>
      ...
      <div className="relative flex h-full w-full max-w-2xl flex-col bg-brand-light/40 shadow-2xl motion-safe:animate-in motion-safe:slide-in-from-right motion-safe:duration-300">
        ...
        <div className="flex-1 space-y-6 overflow-y-auto px-6 py-5">
          {/* Read the files — jump to the full readable project view. When the
              run is finished (draft_ready / published) this becomes a clear
              "your product is ready" handoff with the CTA elevated; while it's
              still working it stays a quiet utility link. */}
          {detail.artifacts.length > 0 &&
            (state === 'draft_ready' || state === 'published' ? (
              <ProductReadyCard runId={detail.id} published={state === 'published'} />
            ) : (
              <Link
          ...
          <section aria-labelledby="pipeline-h">
            ...
            <ol className="space-y-2.5">
              {ACTIVE_STAGES.map((s) => {
                ...
                // Stage-3 build only: if it carries a computational workbook, the
                // gate audited its live formulas — surface that as a trust lens.
                const workbook =
                  s.n === 3
...
/**
 * The finished-product handoff. Shown only on a TERMINAL done state
 * (draft_ready / published): the moment the factory's work is a deliverable, so
 * the read/download CTA earns top billing instead of sitting as a plain row. The
 * copy is honest — it's "ready to read & ship", not a claim of revenue — and the
 * one entrance flourish is motion-safe (no animation under prefers-reduced-motion).
 */
function ProductReadyCard({ runId, published }: { runId: string; published: boolean }) {
  return (
    <div className="overflow-hidden rounded-xl border border-emerald-200 bg-gradient-to-br from-emerald-50 to-brand-light/60 p-5 shadow-sm motion-safe:animate-in motion-safe:fade-in motion-safe:zoom-in-95 motion-safe:duration-300">
      <div className="flex items-start gap-3">
        <span
          className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-emerald-100 text-emerald-700"
          aria-hidden="true"
        >
        ...
        <div className="min-w-0">
          ...
          <h3 className="mt-0.5 text-base font-semibold text-brand-accent">
            Your product is ready
          </h3>
          <p className="mt-1 text-sm text-brand-dark/65">
            Every stage is complete. Read the full project, then download the document
            {published ? ' — and the working files.' : ' and any working files.'}
          </p>
        </div>
      </div>
      <Link
        href={`/factory/${runId}`}
        className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-emerald-700 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50"
      >
        Read &amp; download the product
...
/**
 * Spreadsheet gate-lens — the "the gate audited the REAL deliverable" trust
 * signal for a computational Stage-3 build. Shows the persisted formula-cell
 * count + sheet count (real numbers, never a model claim) and, on demand, a few
 * representative formulas as proof. An HONEST guard: a computational workbook with
 * 0 live-formula cells renders a warning tone, not a green pass — we never fake
 * a verdict the data doesn't support.
 */
        ...
        <div className="min-w-0 flex-1">
          ...
          {audited ? (
            <p className="mt-0.5 text-xs leading-snug text-brand-dark/65">
              Live formulas audited — <strong className="font-semibold text-emerald-700">{cellLabel}</strong>{' '}
              across {sheetLabel}.
            </p>
          ) : (
